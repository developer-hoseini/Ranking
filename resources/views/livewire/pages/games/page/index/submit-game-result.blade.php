        ...
        <div class="result-user-info">
            <a href="{{ route('profile.show',['user'=>$user?->id]) }}" target="_blank"
               title="{{$user?->avatar_name}}">
                <img src="{{ $user?->avatar }}" class="user_photo mb-2" width="80" alt="{{$user?->avatar_name}}">
                <div>{{ $user?->avatar_name }}</div>
            </a>
            <div wire:ignore>{{__('words.Rank: ')}} {{ $user?->game_rank }}</div>
            <div wire:ignore>{{__('words.Score: ')}} {{ $user?->user_score_achievements_sum_count }}</div>
        </div>
