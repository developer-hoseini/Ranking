...
class NoRepeatClubTeam implements ValidationRule
{
    public function __construct(public int $teamId, public int $teamSelectId)
    {
    }
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        if ($value) {
                ...
                ->where(function ($q) {
                    $q->orWhere(function ($q1) {
                        $q1->where('inviter_user_id', auth()->id())
                            ->where('invited_user_id', $this->teamId);
                    })->orWhere(function ($q2) {
                        $q2->where('invited_user_id', auth()->id())
                            ->where('inviter_user_id', $this->teamSelectId);
                    });
                })
            ...
            if ($lastSendCount > 0) { // Yes
                $fail(__('message.days_not_take_or_latest_team_played_not_enough', ['days_count' => config('setting.no_repeat_days'),
